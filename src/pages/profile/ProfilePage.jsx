...
const ProfilePage = () => {
  ...
  const {
    ...
    queryFn: async () => {
      return await apiFetch(`/api/users/profile/${username}`);
    },
  });
  ...
  if (isError) return <p className="text-center text-lg mt-4 text-red-500">{error.message}</p>;
  if (!user) return <p className="text-center text-lg mt-4">User not found</p>;

  return (
    <div className="flex-[4_4_0] border-r border-gray-700 min-h-screen">
      <div className="flex gap-10 px-4 py-2 items-center">
        <FaArrowLeft className="w-4 h-4 cursor-pointer" onClick={() => navigate(-1)} />
